...
"""Script to perform MFCC recognition using HTK.

Originally written by Bryan Quill. Modified by Ashley Gillman.
"""
...
# My def'ns
ORIG_TEST_DATA_DIR = "/Users/Ash/Documents/ThesisData/testDat/"
TEST_NAMES = ["3", "4", "5", "6", "7", "8", "9", "10"]
MAPPING = [('phonememodifiedOnline', 'poo'),
           ('phonememodifiedSupervised', 'pos'),
           ('phonememohammadiaOnline', 'pmo'),
           ('phonememohammadiaSupervised','pms'),
           ('mohammadiaOnline', 'umo'),
           ('mohammadiaSupervised', 'ums')]
HTK_TOOL_DIR = "/Users/Ash/Documents/ThesisData/htk/HTKTools"
...
def doInitialise():
    global command, wavConvertFile, convertFile, conversionCommand, ext
    buildFileStructure()
    # Create WordList
...
def doEvaluate():
    global wavConvertFile, convertFile, conversionCommand, ext, hmmDef, listFile, command, MLFOutput
    ...
    for ext in CLASSIFIER_EXTS:
        ...
        command = str.format("HResults -p -d 5 -f -I {0} {1} {2} > {3}{4}Output.txt",
                             MLF_EVAL_PHONE,
                             SORTED_PHONELIST_LOC,
                             MLFOutput,
                             RESULTS_DIR,
...
def collateData():
    ...
    for test in TEST_NAMES:
        # clean
        name = "test_clean"
        wavfile = str.format("{0}{1}{2}{3}.wav", ORIG_TEST_DATA_DIR, test, SEPARATOR, name)
        wrdfile = str.format("{0}{1}{2}{3}.wrd", ORIG_TEST_DATA_DIR, test, SEPARATOR, name)
        phnfile = str.format("{0}{1}{2}{3}.phn", ORIG_TEST_DATA_DIR, test, SEPARATOR, name)
        shutil.copyfile(wavfile,
                        str.format("{0}c3s{1}{2}{3}.wav", EVAL_AUDIO_DIR, SEPARATOR, test, name))
        shutil.copyfile(wrdfile,
                        str.format("{0}c3s{1}{2}{3}.wrd", EVAL_AUDIO_DIR, SEPARATOR, test, name))
        shutil.copyfile(phnfile,
                        str.format("{0}c3s{1}{2}{3}.phn", EVAL_AUDIO_DIR, SEPARATOR, test, name))

        # dirty
        for root, subdirs, files in os.walk(str.format("{0}{1}{2}", ORIG_TEST_DATA_DIR, test, SEPARATOR)):
            for file in files:
                [name,ext] = file.split(("."))
                if ext == "wav" and name.startswith("test_dirty"):
                    wavfile = str.format("{0}{1}{2}{3}.wav", ORIG_TEST_DATA_DIR, test, SEPARATOR, name)
                    wrdfile = str.format("{0}{1}{2}{3}.wrd", ORIG_TEST_DATA_DIR, test, SEPARATOR, name)
                    phnfile = str.format("{0}{1}{2}{3}.phn", ORIG_TEST_DATA_DIR, test, SEPARATOR, name)
                    shutil.copyfile(wavfile,
        ...
        # enhanced
        for root, subdirs, files in os.walk(str.format("{0}{1}{2}enhanced{2}", ORIG_TEST_DATA_DIR, test, SEPARATOR)):
            for file in files:
                [name,ext] = file.split(("."))
                if ext == "wav":
                    for k, v in MAPPING:
                    ...
                    # wav
                    shutil.copyfile(str.format("{0}{1}{2}enhanced{2}{3}", ORIG_TEST_DATA_DIR, test, SEPARATOR, file),
                                    str.format("{0}c3s{1}{2}{3}.wav", EVAL_AUDIO_DIR, SEPARATOR, test, name))
                    # wrd
                    shutil.copyfile(wrdfile,
                                    str.format("{0}c3s{1}{2}{3}.wrd", EVAL_AUDIO_DIR, SEPARATOR, test, name))
                    ...
                    shutil.copyfile(phnfile,
                                    str.format("{0}c3s{1}{2}{3}.phn", EVAL_AUDIO_DIR, SEPARATOR, test, name))
                    print(str.format("{0}{1}", test, name))


os.environ["PATH"] += os.pathsep + HTK_TOOL_DIR
